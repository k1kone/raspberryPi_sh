...
class App:
    ...
    def __init__(self, master=None):
        self.master = master
        #時刻設定
        self.time_get = datetime.datetime.now()
        ...
        #時刻設定Spinboxの生成
        tk.Label(self.frame1,textvariable=self.time_date).grid(row=0, column=0)
        ...
        self.dht_lb = tk.Label(self.frm_dht, textvariable = self.dht_val, width=15).grid(row=0, column=0)
        ...
        self.grp_t = list()
        self.grp_h = list()
        self.cv = tk.Canvas(width=400, height=200)
    ...
    def show_graph(self):
        self.cv.create_line(10,50,30,80)
        self.con.write(bytes('r\n','utf-8'))
        buf = self.con.readline()
        ardclk = ast.literal_eval(buf.decode())
        self.grp_t.append(float(ardclk['TMP']))
        self.grp_h.append(float(ardclk['RH']))
        if len(self.grp_t) > 4 :
            del self.grp_t[0]
            del self.grp_h[0]
            for i, j in zip(self.grp_t, self.grp_h):
                pass
        root.after(1000, self.show_graph)
            

    def time_disp(self):
        self.time_get = datetime.datetime.now()
        self.time_date.set(self.time_get.strftime('%Y/%m/%d/%H:%M:%S'))
        root.after(1000, self.time_disp)
    #時刻設定「送信」ボタン処理    
    def asubmit(self):

        cmd = 'a,' + self.time_get.strftime('%Y,%m,%d,%H,%M,%S') + '\n'
        buf = bytes(cmd,'utf-8')
        self.con.write(buf)
        print(buf)
